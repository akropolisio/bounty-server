        ...
        match result {
            Ok(_) => {
                ...
                let user = {
                    ...
                    let result = users
                        .filter(address.eq(&data.0.address))
                        .first::<User>(&conn);
                    match result {
                        Ok(user) => {
                        ...
                        Err(err) => {
                            ...
                            return HttpResponse::NotFound()
                                .json(api::ApiError::UserNotFound.to_resp());
                        }
                    }
                };
...
fn recaptcha_future(
    ...
    let url = {
        ...
        let addr: Option<std::net::IpAddr> =
            match req.connection_info().remote().map(|addr| addr.parse()) {
    ...
    client
        ...
        .and_then(|mut response| {
            ...
            response
                .json::<recaptcha::RecaptchaResponse>()
                .from_err()
                .and_then(|response| {
...
fn create_token(old_token: web::Query<api::OldToken>, req: HttpRequest) -> impl Responder {
    ...
    let headers = req
        ...
        .map(|(k, v)| {
            (
                k.as_str().to_string(),
                v.to_str()
                    .unwrap_or("the header value has not visible ASCII chars")
                    .to_string(),
            )
        })
        .collect();
    ...
    diesel::insert_into(logs::table)
        .values(NewLog {
            ...
            payload: PayloadWrapper(headers),
        })
        .execute(&conn)
        .expect("Error saving new log record");

    HttpResponse::Ok().json(api::Token {
        status: "ok".to_string(),
        token,
    })
}
...
fn log_action(log: web::Json<api::Log>, req: HttpRequest) -> impl Responder {
    ...
    use db::models::Token;
    use db::schema::tokens::{self, expired_at, token};
    use diesel::prelude::{ExpressionMethods, QueryDsl, RunQueryDsl};
        ...
        Ok(current_token) => {
            ...
            diesel::insert_into(logs::table)
                .values(NewLog {
                    token: &log.token,
                    action: &log.action,
                    payload: PayloadWrapper(log.payload.clone()),
                })
                .execute(&conn)
                .expect("Error saving new log record");

            HttpResponse::Ok().json(api::StatusOk {
                status: "ok".to_string(),
            })
        }
        Err(err) => {
            log::debug!("token not found, reason: {:?}", err);
            HttpResponse::Unauthorized().json(api::StatusError {
